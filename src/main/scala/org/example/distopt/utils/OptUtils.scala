package org.example.distopt.utils

import breeze.linalg.{SparseVector, Vector, norm, sum}
...
object OptUtils {
  ...
  def computePrimalObjective(data: RDD[LabeledPoint], w: Vector[Double], lambda: Double): Double = {
    return computeAvgLoss(data, w) + (0.5 * lambda * Math.pow(norm(w), 2))
  }
  ...
  def computeDualObjective(data: RDD[LabeledPoint], w: Vector[Double], alpha : RDD[Vector[Double]], lambda: Double): Double = {
    val n = data.count()
    val sumAlpha = alpha.map(x => sum(x)).reduce(_ + _)
    return (-lambda / 2 * Math.pow(norm(w), 2)) + (sumAlpha / n)
  }

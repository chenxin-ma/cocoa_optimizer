...
object driver {
  ...
  def main(args: Array[String]) {
    ...
    val numSplits = 4 //options.getOrElse("numSplits","1").toInt
    val chkptDir = options.getOrElse("chkptDir","");
    var chkptIter = options.getOrElse("chkptIter","100").toInt

    // algorithm-specific inputs
    val lambda = options.getOrElse("lambda", "0.002").toDouble // regularization parameter
    val numRounds = options.getOrElse("numRounds", "20").toInt // number of outer iterations, called T in the paper
    val localIterFrac = options.getOrElse("localIterFrac","0.2").toDouble; // fraction of local points to be processed per round, H = localIterFrac * n
    ...
    val n = trainData.count().toInt // number of data examples

    // compute H, # of local iterations
    var localIters = 10000//(localIterFrac * n / trainData.partitions.size).toInt
    print(localIters)
